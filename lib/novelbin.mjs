...
class SiteTemplate {
  ...
  async searchNovels(searchTerm, pageNo = 1) {
    ...
    const url  = `${this.site.replace(/\/$/, '')}/ajax/search-novel?keyword=${term}`;
    ...
    return $('a.list-group-item')
      .map((_, ele) => {
        const name = $(ele).attr('title')?.trim() || $(ele).text().trim();
        const path = $(ele).attr('href') || '';
        
        // Skip "See more results" link
        if (!path || path.includes('/search?keyword=') || path.includes('/searchkeyword=')) return null;

        // Normalize path
        const relativePath = path
          .replace(this.site, '')
          .replace(/^https?:\/\/novelbin\.(com|me|net|org)\//, '')
          .replace(/^\//, '');

        return { 
          name, 
          cover: '', // AJAX search doesn't provide covers
          path: relativePath 
        };
      })
      .get()
  ...
  async searchNovelsInternal(searchTerm, pageNo = 1, type = '') {
    // This method is kept for backward compatibility or full search if needed,
    // but the main searchNovels now uses the AJAX logic.
    const term = encodeURIComponent(searchTerm.replace(/\s+/g, '+'));
