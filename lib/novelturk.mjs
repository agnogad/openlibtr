...
class NovelTurk {
  ...
  async parseNovel(novelPath) {
    ...
    // Kapak resmini label sayfasından al (search/label)
    let cover = '';
    try {
      const labelUrl = `${this.site}/search/label/${encodeURIComponent(labelName)}`;
      const labelBody = await fetchApi(labelUrl).then(r => r.text());
      const $label = parseHTML(labelBody);
      
      // .thumb class'lı img'yi bul (label sayfasında doğru kapak)
      const thumbImg = $label('img.thumb').first();
      if (thumbImg.length) {
        cover = thumbImg.attr('src') || '';
      }
      
      // w140 küçük thumbnail, w500 büyük versiyon al
      if (cover && cover.includes('/w140/')) {
        cover = cover.replace('/w140/', '/w500/');
      }
      
      // Uzantıyı bozmadan tut - Blogger zaten doğru formatta servis ediyor
  ...
  async searchNovels(searchTerm, pageNo = 1) {
    ...
    try {
      ...
      let cover = '';
      try {
        const labelUrl = `${this.site}/search/label/${encodeURIComponent(searchTerm)}`;
        const labelBody = await fetchApi(labelUrl).then(r => r.text());
        const $label = parseHTML(labelBody);
        
        // .thumb class'lı img'yi bul
        const thumbImg = $label('img.thumb').first();
        if (thumbImg.length) {
          cover = thumbImg.attr('src') || '';
        }
