...
class NovelTurk {
  ...
  async parseNovel(novelPath) {
    ...
    // Kapak resmini ilk bölüm sayfasından al (500x500 boyutlu olan)
    let cover = '';
    if (chapters[0]?.path) {
      try {
        ...
        // 500x500 boyutlu resmi bul (kapak genellikle bu boyutta)
        $ch('img').each((_, el) => {
          const w = $ch(el).attr('width');
          const h = $ch(el).attr('height');
          if (w === '500' && h === '500') {
        ...
        // Alternatif: header içindeki img
        if (!cover) {
          cover = $ch('header img').first().attr('src') || '';
        }
